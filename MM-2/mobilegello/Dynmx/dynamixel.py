...
class Dynamixel:
    ...
    def velocity_trajectory(self):
        ...
        for DXL_ID in range(1, 7):
            # Read the Velocity Trajectory (Address 136)
            velocity_ = self.packetHandler.read4ByteTxRx(self.portHandler, DXL_ID, 136)
            if velocity_[0] == COMM_SUCCESS:
                # Append the actual value of the trajectory, not the status
                velocity_trajectory.append(velocity_[1])
            else:
                print(f"Error reading velocity trajectory for ID {DXL_ID}: {velocity_[2]}")
        return velocity_trajectory

    def position_trajectory(self):
        position_trajectory = []
        for DXL_ID in range(1, 7):
            # Read the Position Trajectory (Address 140)
            position_ = self.packetHandler.read4ByteTxRx(self.portHandler, DXL_ID, 140)
            if position_[0] == COMM_SUCCESS:
                # Append the actual value of the trajectory, not the status
                position_trajectory.append(position_[1])
            else:
                print(f"Error reading position trajectory for ID {DXL_ID}: {position_[2]}")
    ...
    def _disable_torque(self, motor_id):
        # print(f'disabling torque for motor {motor_id}')
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, motor_id,
    ...
    def set_profile_acceleration(self, motor_id: int, acceleration: int):
        dxl_comm_result, dxl_error = self.packetHandler.write4ByteTxRx(self.portHandler, motor_id,
                                                                       self.ADDR_PROFILE_ACCELERATION, acceleration)
        self._process_response(dxl_comm_result, dxl_error, motor_id)
